...
export default function HeroSection() {
  ...
  const contentScale = useTransform(smoothScrollY, [0, 400], [1, 0.9]);

  // 2. Blur background (progressive)
  const blurAmount = useTransform(smoothScrollY, [0, 600], ["blur(0px)", "blur(12px)"]);

  // 3. Darken background (progressive)
  const overlayOpacity = useTransform(smoothScrollY, [0, 600], [0, 0.6]);
  const parallaxY = useTransform(smoothScrollY, [0, 1000], [0, 400]);

  // Hero images from S3
  const imageSources = heroImages;

  // (Removed manual scroll listener)
  ...
  return (
    ...
    >
      ...
      {/* Parallax Overlay - Darkens as you scroll */}
      <motion.div
      ...
      <motion.div
        ...
        style={{
          ...
          padding: "15px 0",
          opacity: contentOpacity, // Link opacity to scroll
        }}
      >
        <div
